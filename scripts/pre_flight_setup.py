...
def step_2b_install_pip_in_venv(python_in_venv):
    ...
    pip_installed = run_command(pip_check_command, check=False) # check=False because we expect it to fail if pip is missing
    
    if pip_installed:
        print("pip is already installed in the VENV.")
    else:
        print("pip not found in VENV. Attempting to install it with get-pip.py...")
...
if __name__ == "__main__":
    ...
    step_2b_install_pip_in_venv(python_in_venv) # New step to ensure pip exists
