...
try:
    ...
    NEW_MAIN_GRADIO_APP_VERSION = "1.1.0"
...
try:
    ...
    # Exit if core modules are missing, as the app is non-functional.
    sys.exit(1)
...
def get_asset_choices(sd_version, inpainting_filter_mode="Show All Models"):
    ...
    filtered_models = [name for name, data in models_src.items() if
                       (inpainting_filter_mode == "Show All Models") or
                       (inpainting_filter_mode == "Inpainting Models Only" and data.get("inpainting")) or
                       (inpainting_filter_mode == "No Inpainting Models" and not data.get("inpainting"))]
...
def update_all_ui_elements(sd_version_selected, inpainting_filter_selected, webui_selected):
    ...
    return (
        ...
        gr.update(choices=controlnets, value=[]), gr.update(choices=loras, value=[]),
...
def launch_anxlight_main_process(webui_choice, sd_version, models_selected, vaes_selected,
    ...
    try:
        ...
        config_data = {
            ...
            'selected_assets': {
                'models': models_selected, 'vaes': vaes_selected,
                'controlnets': controlnets_selected, 'loras': loras_selected
            },
            'tokens': {
                'civitai': civitai_token, 'huggingface': huggingface_token,
                'zrok': zrok_token, 'ngrok': ngrok_token
            },
            'launch_args': custom_args,
            'options': {
                'update_webui': update_webui, 'update_extensions': update_extensions,
                'check_deps': check_custom_nodes_deps, 'detailed_log': detailed_download
            }
        }
        ...
        if any(config_data['selected_assets'].values()):
            ...
            # Assuming Manager.py has a function that yields progress
        ...
        for line in iter(launch_process.stdout.readline, ''):
            if line.strip():
                yield line # Stream output directly
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Tabs():
            with gr.TabItem("Session Configuration"):
                ...
                theme_accent_dd = gr.Dropdown(label="UI Theme", choices=THEME_CHOICES, value=THEME_CHOICES[0], visible=False) # Hidden for now
        ...
        launch_button.click(
            ...
            inputs=[webui_choice_dd, sd_version_rb, models_cbg, vaes_cbg, controlnets_cbg, 
                    ...
                    zrok_token_tb, ngrok_token_tb, custom_args_tb, theme_accent_dd, inpainting_filter_rg],
            outputs=live_log_ta
        )
    return demo
